#!/usr/bin/env bash
# Generate and read back one Qwen2.5-Omni-7B N=4 resampling shard.
set -euo pipefail
...
for gpu in "${lock_gpus[@]}"; do
  ...
  echo "[resample shard=$shard] waiting MiniCPM lock gpu=$gpu host=$(hostname)"
  flock "$fd"
...
while :; do
  ...
  echo "[resample shard=$shard] GPUs busy: g${gpu_a}=${used_a}MiB g${gpu_b}=${used_b}MiB"
  sleep 30
done
...
limit=$(( end - offset ))
tag="$(hostname -s)_g${gpu_a}-${gpu_b}_s${shard}of${num_shards}"
...
export TRITON_CACHE_DIR="/tmp/triton_qwen25_7b_resample_${USER}_g${gpu_a}-${gpu_b}"
...
run_generation() {
  apptainer exec --nv --bind /workspace:/workspace "$image" bash -lc '
    ...
    exec "$QWEN_PYTHON" -u scripts/analyze/generate_vllm_omni_resamples.py \
      ...
      --offset "$QWEN_OFFSET" --limit "$QWEN_LIMIT" --seeds 101,202,303 \
...
if ! run_generation; then
  echo "[resample shard=$shard] retrying missing candidates" >&2
  run_generation
fi
...
"$asr_python" -u scripts/facts/readback_resample.py \
  ...
  --offset "$offset" --limit "$limit" --n 3 --channels asr1,asr2 \
...
run_funasr() {
  env PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
    ...
    --model Qwen2.5-Omni-7B --device cuda:0 \
    --num-shards "$num_shards" --shard-index "$shard"
}
if ! run_funasr; then
  echo "[resample shard=$shard] retrying missing FunASR readbacks" >&2
  run_funasr
fi
printf 'host=%s gpu=%s completed=%s\n' \
  "$(hostname)" "$gpu_a" "$(date --iso-8601=seconds)" \
  >"$barrier_root/readback-s${shard}-of-${num_shards}.done"
echo "[resample shard=$shard] complete=$(date --iso-8601=seconds)"
