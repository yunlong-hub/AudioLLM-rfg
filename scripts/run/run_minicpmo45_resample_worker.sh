#!/usr/bin/env bash
# One resumable generation + three-ASR shard for MiniCPM-o N=4.
set -euo pipefail
...
log_dir="$root/output/minicpmo45/resample-logs/$barrier_tag"
mkdir -p "$barrier_dir" "$log_dir"
...
echo "[resample-worker] host=$(hostname) gpu=$gpu shard=$shard/$num_shards range=$offset:$end"
...
echo "[resample-worker] completed host=$(hostname) shard=$shard/$num_shards"
